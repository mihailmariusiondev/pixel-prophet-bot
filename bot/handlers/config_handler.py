...
async def config_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            # Check limits
            if (
                value < ALLOWED_PARAMS[param]["min"]
                or value > ALLOWED_PARAMS[param]["max"]
            ):
                raise ValueError(
            ...
            logging.warning(
                f"Invalid value for parameter {param} from user {user_id}: {value}"
            )
            await update.message.reply_text(
                f"❌ Valor no válido para {param}.\n"
                f"Debe ser un número entre {ALLOWED_PARAMS[param]['min']} y {ALLOWED_PARAMS[param]['max']}"
            )
            return
